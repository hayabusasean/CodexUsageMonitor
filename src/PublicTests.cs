...
internal static class PublicTests {
 public static void Run(string[] args){
  ...
  Check("O07 high contrast respects system colors without changing settings",()=>{var normal=Theme.Canvas;try{Theme.TestHighContrast=true;Assert(Theme.Canvas==SystemColors.Window&&Theme.Text==SystemColors.WindowText);using var service=new MonitorService(Path.Combine(root,"contrast"));using var overlay=new OverlayForm(service,true);overlay.ApplySettings();Assert(overlay.Opacity==1&&service.Settings.OpacityPercent==70);}finally{Theme.TestHighContrast=null;}Assert(Theme.Canvas==normal);});

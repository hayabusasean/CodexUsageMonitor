...
internal sealed class DarkChoice:Control {
 internal int LogicalWidth=160;public readonly List<object> Items=[];int selected=-1;bool hover;Form? popup;public event EventHandler? SelectedIndexChanged;
 ...
 public DarkChoice(){BackColor=Theme.Canvas;Size=new(160,36);TabStop=true;DoubleBuffered=true;Cursor=Cursors.Hand;AccessibleRole=AccessibleRole.ComboBox;}
 protected override void OnMouseEnter(EventArgs e){hover=true;Invalidate();base.OnMouseEnter(e);}
 protected override void OnMouseLeave(EventArgs e){hover=false;Invalidate();base.OnMouseLeave(e);}
 protected override void OnPaint(PaintEventArgs e){var g=e.Graphics;Theme.PaintControlBackground(this,g);float k=DeviceDpi/96f;using var path=Theme.Round(new RectangleF(1,1,Width-2,Height-2),8*k);using var fill=new SolidBrush(hover&&Enabled?Theme.Surface3:Theme.Surface2);using var border=new Pen(Focused?Theme.Accent:Theme.Border);g.FillPath(fill,path);g.DrawPath(border,path);
 ...
 internal void OpenChoices(){if(popup!=null){popup.Close();return;}int row=(int)(36*DeviceDpi/96f);popup=new Form{FormBorderStyle=FormBorderStyle.None,ShowInTaskbar=false,StartPosition=FormStartPosition.Manual,BackColor=Theme.Surface,TopMost=true,Size=new(Width,Math.Min(Items.Count,10)*row+8)};
  ...
  for(int i=0;i<Math.Min(Items.Count,10);i++){int ix=i;var b=Theme.Button(Items[i].ToString()??"",(_,_)=>{SelectedIndex=ix;p.Close();});b.Bounds=new(4,4+i*row,Width-8,row);b.Font=Font;((QuietButton)b).Selected=i==SelectedIndex;((QuietButton)b).Role=ButtonRole.Ghost;p.Controls.Add(b);}
...
internal sealed class DarkCheck:CheckBox {
 public DarkCheck(){BackColor=Theme.Canvas;SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer,true);FlatStyle=FlatStyle.Flat;}
 public override Size GetPreferredSize(Size proposedSize){var text=TextRenderer.MeasureText(Text,Font);return new(text.Width+(int)((R2.Is(this)?46:32)*DeviceDpi/96f),Math.Max(text.Height+8,(int)(28*DeviceDpi/96f)));}
 protected override void OnPaint(PaintEventArgs e){if(R2.Is(this)){R2.Switch(this,e.Graphics);return;}var g=e.Graphics;Theme.PaintControlBackground(this,g);float k=DeviceDpi/96f;float side=14*k,y=(Height-side)/2;using var path=Theme.Round(new(1,y,side,side),3*k);using var b=new SolidBrush(Checked?Theme.Accent:Theme.Surface);using var p=new Pen(Focused?Theme.Accent:Theme.Muted);g.FillPath(b,path);g.DrawPath(p,path);
...
internal sealed class ScrollRail:Control {
 readonly Panel panel;bool held,syncing;int drag,initial; (int,int,int) lastScroll;
 ...
 public void Sync(){
  if(IsDisposed||!panel.IsHandleCreated||syncing)return;
  syncing=true;try{
   if(panel is SpacePanel sky)sky.HideNativeScroll();
   bool overflow=panel.DisplayRectangle.Height>panel.ClientSize.Height+2;
   if(Visible!=overflow)Visible=overflow;
   if(Parent!=null){var bounds=new Rectangle(panel.Right-12,panel.Top+8,12,Math.Max(10,panel.Height-16));if(Bounds!=bounds)Bounds=bounds;if(Parent.Controls.GetChildIndex(this)!=0)BringToFront();}
   var state=(panel.DisplayRectangle.Height,panel.ClientSize.Height,panel.AutoScrollPosition.Y);if(lastScroll!=state){lastScroll=state;Invalidate();}

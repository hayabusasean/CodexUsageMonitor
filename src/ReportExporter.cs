...
internal static class ReportExporter {
 ...
 public const string Version=L.Version;public const string SchemaVersion="4";const long VolumeBytes=8*1024*1024;
 public static string Prompt(string? reportLanguage=null)=>L.TFor(reportLanguage=="zh-TW"?"zh-TW":reportLanguage==null?L.Language:"en-US","Report.Prompt")+((reportLanguage??L.Language)=="zh-TW"?"\n本機首次觀察時間不等於伺服端實際執行時間或社群推估時間。WATCH 是注意力提示，不是重置預測。delta_pp 是百分點，不是收益百分比。舊週期本機提醒退場不改寫外部 phase；原因維持 UNKNOWN。":"\nLocal first observation is not server execution time or community-estimated timing. WATCH is an attention signal, not a reset prediction. delta_pp means percentage points, not percentage profit. Retiring an old local-cycle reminder does not rewrite external phase; cause remains UNKNOWN.");
 ...
 public static string Export(MonitorService s,HistoryFilter filter,bool notes,CancellationToken ct,string? destination=null,string? reportLanguage=null,UiDpi? displayDpi=null){
  ...
  var ev=frozen.Rows<UsageEvent>().Where(x=>x.first_observed_at_utc>=filter.From&&x.first_observed_at_utc<=end&&(filter.Pool==""||x.limit_id==filter.Pool)&&(!filter.Duration.HasValue||x.window_duration_mins==filter.Duration)).ToList();
   var radar=s.Radar.Snapshot();var focus=EventFocusEvidence.FocusEvents(ev).Select(x=>EventFocusEvidence.Build(x,input,radar,asOf)).ToList();
  ...
  try{Open();
   ...
   Section("RADAR_SOURCE_HEALTH",One(new{record_type="SUMMARY",coverage=Rc4Radar.Coverage(radar,s.Radar.Enabled,asOf),enabled=s.Radar.Enabled,configured_sources=ResetRadar.Sources.Length,healthy_sources=Rc4Radar.HealthyConfiguredCount(radar,asOf),failed_sources=s.Radar.FailedSourceCount}).Concat(Rc4Radar.LatestConfiguredSources(radar).Select(x=>(object)new{record_type="SOURCE",source=privacy.Source(x)})));
   var coverageRows=radar.CoverageTimeline.Where(x=>x.timestamp>=filter.From&&x.timestamp<=end).OrderBy(x=>x.timestamp).ToList();
   Section("RADAR_COVERAGE_TIMELINE",coverageRows.Cast<object>());
   var currentCoverage=RadarCoverage.Measure(radar,s.Radar.Enabled,asOf);var numeric=coverageRows.Where(x=>x.coverage_percent!=null).ToArray();
   Section("RADAR_SOURCE_BREAKDOWN",One(new{record_type="SUMMARY",current=currentCoverage,sweep_count=coverageRows.Count,numeric_sweep_count=numeric.Length,
    average_coverage_percent=numeric.Length>0?numeric.Average(x=>(double)x.coverage_percent!.Value):(double?)null,minimum_coverage_percent=numeric.Length>0?numeric.Min(x=>x.coverage_percent):null,
    possible_observation_gap=numeric.Length>0?(bool?)numeric.Any(x=>x.readable_sources<x.enabled_sources):null,interpretation="Sweep-sampled mean, not time-weighted. Reduced coverage is a possible observation gap, never proof of a missed signal. Missing historical coverage remains unknown."})
    .Concat(ResetRadar.Sources.Select(d=>(object)new{record_type="SOURCE",source_id=d.Id,source_class=d.SourceClass,
     attempts=radar.SourceChecks.Count(x=>x.SourceId==d.Id&&x.Timestamp>=filter.From&&x.Timestamp<=end),
     failed_attempts=radar.SourceChecks.Count(x=>x.SourceId==d.Id&&x.Timestamp>=filter.From&&x.Timestamp<=end&&x.Status is not ("HEALTHY" or "UNCHANGED")),
     readable_now=Rc4Radar.LatestConfiguredSources(radar).Any(x=>x.Id==d.Id&&s.Radar.Enabled&&RadarCoverage.Readable(x,d,asOf)),fresh_window_seconds=RadarCoverage.FreshWindow(d).TotalSeconds})));
   var signalHistory=radar.SignalHistory.Concat(radar.Events.Where(Rc4Radar.IsAlert).Where(e=>!radar.SignalHistory.Any(h=>h.EventId==e.EventId&&h.RevisionId==e.RevisionId)).Select(x=>new RadarSignalEvidence(x.FirstSeen,x.EventId,x.RevisionId,x.SignalLevel,x.Phase,x.Effect,x.Scope,x.ResetUtc,x.TimeQuality,x.PublishedAt,x.ReadAt,x.Source,x.SourceClass,x.OriginalSourceUrl,x.RelayUrl,x.OriginalQuote,x.ContentHash,x.SignalReason))).OrderBy(x=>x.Timestamp).ToList();
   Section("RADAR_SIGNALS",signalHistory.Select(x=>(object)privacy.Signal(x)));
   Section("RADAR_TRANSITIONS",radar.Transitions.Select(x=>(object)new{x.EventId,x.RevisionId,x.Timestamp,x.FromSignal,x.ToSignal,x.Reason}));
   Section("RESET_FIELD_TRIAL_TIMELINE",
    radar.SourceChecks.Select(x=>(object)new{timestamp=x.Timestamp,record_type="SOURCE_CHECK",id=x.SourceId,source_class=x.SourceClass,state=x.Status,candidate_count=x.CandidateCount,http_status=x.HttpStatus,body_hash=x.BodyHash,last_success=x.LastSuccess,next_check=x.NextCheck,detail=x.Error})
    .Concat(signalHistory.Select(x=>(object)new{timestamp=x.Timestamp,record_type="SIGNAL",id=x.EventId,state=x.SignalLevel,detail=x.SignalReason,signal=privacy.Signal(x)}))
    .Concat(radar.Transitions.Select(x=>(object)new{timestamp=x.Timestamp,record_type="TRANSITION",id=x.EventId,state=x.ToSignal,detail=x.Reason}))
    .Concat(radar.QuotaEvidence.Select(x=>(object)new{timestamp=x.CapturedAt,record_type="LOCAL_QUOTA",id=x.EventId,state=x.WeeklyAfter>x.WeeklyBefore?"REPLENISHMENT_OBSERVED":x.ScopeAssurance=="STABLE_SCOPE"&&x.BankedBefore.HasValue&&x.BankedAfter.HasValue&&x.BankedBefore!=x.BankedAfter?"BANKED_RESET_MOVEMENT":"OBSERVATION",detail="Cause remains UNKNOWN"}))
    .OrderBy(x=>JsonSerializer.Serialize(x)));
   Section("LOCAL_QUOTA_AROUND_SIGNAL",radar.QuotaEvidence.Select(x=>(object)privacy.Quota(x)));
   Section("CORRELATION",radar.Correlations.Select(x=>(object)new{x.PreviousSample,x.CurrentSample,x.From,x.To,x.Before,x.After,x.Classification,x.Reason,x.AnnouncementId,x.ScopeVerified,x.Scheduled,x.AnnouncementStatus,x.LocalObservation,x.Correlation,Cause="UNKNOWN"}));
   Section("EVIDENCE_LIMITATIONS",One(new{window_hours=72,source_checks_are_public_best_effort=true,status_incidents_are_supporting_only=true,team_relays_are_not_official_announcements=true,original_x_posts_are_never_fetched_by_the_monitor=true,banked_reset_credits_are_not_automatic_replenishment=true,local_history_is_this_pc_only=true,correlation_is_not_causation=true,cause="UNKNOWN",missing_source_or_quota_evidence_must_remain_unknown=true}));
 ...
 internal sealed class Privacy {
  ...
  public Announcement Announcement(Announcement x)=>x with{Title=Scrub(x.Title),Summary=Scrub(x.Summary),Author=Scrub(x.Author),OriginalId=Scrub(x.OriginalId),CanonicalOriginalId=Scrub(x.CanonicalOriginalId),Url=PublicUrl(x.Url),OriginalSourceUrl=PublicUrl(x.OriginalSourceUrl),RelayUrl=PublicUrl(x.RelayUrl),OriginalQuote=Scrub(x.OriginalQuote),RelayForecast=Scrub(x.RelayForecast),SignalReason=Scrub(x.SignalReason),EvidenceSources=(x.EvidenceSources??[]).Select(e=>e with{Url=PublicUrl(e.Url)}).ToList()};
  ...
  public RadarSignalEvidence Signal(RadarSignalEvidence x)=>x with{OriginalSourceUrl=PublicUrl(x.OriginalSourceUrl),RelayUrl=PublicUrl(x.RelayUrl),OriginalQuote=Scrub(x.OriginalQuote),SignalReason=Scrub(x.SignalReason)};
  public object Quota(RadarQuotaEvidence x)=>new{x.EventId,x.CapturedAt,previous_sample=Alias("sample",x.PreviousSample),current_sample=Alias("sample",x.CurrentSample),x.From,x.To,x.WeeklyBefore,x.WeeklyAfter,x.ResetAtBefore,x.ResetAtAfter,x.BankedBefore,x.BankedAfter,x.ObservationGapSeconds,x.ScopeAssurance,Cause="UNKNOWN"};
